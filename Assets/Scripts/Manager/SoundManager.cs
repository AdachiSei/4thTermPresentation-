...
public class SoundManager : MonoBehaviour
{
    ...
    private int _newAudioSourceCount;
    ...
    async public void PlaySFX(string name, float volume = 1)
    {
        ...
        foreach (var data in _sfxDatas)
        {
            if (data.name == name || data.Name == name)
            {
                ...
                var isPlayingNew = !newAudioSource.isPlaying;
    ...
    /// <param name="volume">音の大きさ</param>
    async public void PlaySFX(string name, AudioSource audioSource, float volume = 1)
    {
        var sfxVolume = volume * _masterVolume * _sfxVolume;
        ...
        foreach (var data in _sfxDatas)
        {
            if (data.name == name || data.Name == name)
            {
                SetClip(ref audioSource, data.SFXClip);
    ...
    /// <summary>
    /// オーディオソースにクリップを設定する関数
    /// </summary>
    private void SetClip(ref AudioSource audioSource,AudioClip audioClip)
    {
